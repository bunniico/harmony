...
- **Webhooks in:** with `webhook_port` set, an aiohttp server (already a discord.py dependency) takes adderall's Discord-style webhooks (`{"content": ...}`, for now new ClickUp assignments) at `/adderall/<ADDERALL_WEBHOOK_TOKEN>`, answers 204 at once, and DMs the content in Harmony's voice. A wrong token is a 404; the bot won't start with the port set and a token under 24 characters; access logging is off so the token never reaches the logs.
